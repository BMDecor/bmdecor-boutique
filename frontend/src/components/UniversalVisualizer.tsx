...
import type { RoomScene } from '@/lib/visualizer/room-scenes';
...
/**
 * UniversalVisualizer — "Marbella Room Set"
 *
 * Brand-agnostic visualizer using CSS mix-blend-mode: multiply to composite
 * any selected paint color onto architectural room scene illustrations.
 * Works for BM, LG, and FB hex codes.
 */
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div
                ...
                style={{ backgroundColor: color.hexCode, mixBlendMode: 'multiply' }}
              />
              <div
                className="absolute inset-0 opacity-20"
                style={{ backgroundColor: color.hexCode, mixBlendMode: 'color' }}
              />
              <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/50 to-transparent p-4">
                <p className="text-white text-sm font-medium">{activeScene?.name}</p>
                <p className="text-white/60 text-xs">{color.name} &middot; {color.colorCode}</p>
              </div>
              <div className="absolute top-3 right-3 bg-black/40 backdrop-blur-sm rounded-lg px-2.5 py-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full border border-white/30" style={{ backgroundColor: color.hexCode }} />
          ...
          <div className="grid grid-cols-6 gap-2">
            {roomScenes.slice(0, 6).map((scene) => (
              ...
              >
                <img src={scene.imageUrl} alt={scene.name} className="absolute inset-0 w-full h-full object-cover" />
                <div className="absolute inset-0" style={{ backgroundColor: color.hexCode, mixBlendMode: 'multiply' }} />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl overflow-hidden shadow-lg">
          ...
          >
            <div className="absolute top-4 left-4 flex items-center gap-2" style={{ color: textMuted }}>
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" />
              </svg>
              <span className="text-[10px] uppercase tracking-widest font-medium">Studio Swatch</span>
            </div>
            <div className="text-center">
              <h2 className="text-2xl font-semibold tracking-tight mb-1" style={{ color: textColor, fontFamily: 'var(--font-playfair)' }}>
                {color.name}
              </h2>
              <p className="text-sm font-mono tracking-wide" style={{ color: textMuted }}>{color.colorCode}</p>
            </div>
            <div className="absolute bottom-4 right-4 text-xs font-mono" style={{ color: textSubtle }}>{color.hexCode}</div>
          </div>
          ...
          <div className="bg-[#2C2C2C] text-white px-5 py-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center">
                ...
                <div className="text-xl font-semibold" style={{ color: accentColor }}>{lrv}</div>
